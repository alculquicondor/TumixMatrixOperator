...
public class InvertMatrix extends Activity {
	private Button result, toinvert;
	private LinearLayout matrixresultview;

	@Override
	public void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		setContentView(R.layout.matrixinverse);
		result = (Button) findViewById(R.id.result_matrix_button);
		toinvert = (Button) findViewById(R.id.matrixtoinvert_button);
	...
	public void onCalculateClick(View view) {
		if (toinvert.getText().toString() == "") {
			Toast.makeText(getBaseContext(),
					R.string.Error_SelectMatrizesOperator, Toast.LENGTH_LONG)
					.show();
			return;
		}
		SquareMatrix matrixtoinvert, ans = null;
		int idtoinvert;
		idtoinvert = toinvert.getText().toString().charAt(0) - 'A';
		matrixtoinvert = Main.matrizes[idtoinvert].toSquare();
		if (matrixtoinvert == null) {
			Toast.makeText(getBaseContext(), R.string.Error_MatrixNotSquare,
					Toast.LENGTH_LONG).show();
			return;
		}
		ans = matrixtoinvert.inverse();
		if (ans == null) {
			Toast.makeText(getBaseContext(), R.string.Error_MatrixSingular,
					Toast.LENGTH_LONG).show();
			return;
		}
